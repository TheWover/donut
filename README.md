...
```
 usage: donut [options] <EXE/DLL/VBS/JS>
       ...
       -e <level>           Entropy. 1=disable, 2=use random names, 3=random names + symmetric encryption (default)
       ...
       -o <outfile_file>    Default is "loader.bin"
       -f <format>          Output format. 1=raw (default), 2=base64, 3=c, 4=ruby, 5=python, 6=powershell, 7=C#, 8=hex
       -y                   Create new thread for entrypoint and return handle to caller. Default uses existing thread.
       ...
       -z <engine>          Pack/Compress file. 1=disable, 2=LZNT1, 3=Xpress, 4=Xpress Huffman

 examples:

    donut c2.dll
    donut -a1 -cTestClass -mRunProcess -pnotepad.exe loader.dll
    donut loader.dll -c TestClass -m RunProcess -p"calc notepad" -u http://remote_server.com/modules/
```
